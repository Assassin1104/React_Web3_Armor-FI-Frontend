...
const EditCoveredModal = ({
  ...
  useEffect(() => {
    const _available = availableCoverage.find(
      (c) => c.address.toLowerCase() === editCoveredValue.address.toLowerCase()
    )
  ...
  useEffect(() => {
    ...
    if (
      editCoveredValue &&
      editCoveredValue.balance &&
      editCoveredValue.balance.eth
    ) {
      setCurrentAmount(editCoveredValue.balance.eth.toString())
      setAmount(editCoveredValue.balance.eth.toString())
    }
    setIsLoading(false)
  }, [editCoveredValue])
  ...
  const contractBalancesReturned = () => {
    ...
    //   (c) =>
    //     c.address.toLowerCase() === editCoveredValue.address.toLowerCase()
    // )
  ...
  const handleSubmitNewValue = async () => {
    ...
    if (
      parseFloat(amount) <= 0 ||
      parseFloat(amount) >
        parseFloat(getCoverAvailable(editCoveredValue.address))
    ) {
      emitter.emit(
        SNACKBAR_ERROR,
        ...
        <ContentContainer>
          ...
          <InfoWrapper>
            <MainInfo>
              ...
              <Value>
                {formatETH(editCoveredValue.detectedBalances.eth.toString(), {
                  compact: true,
                  digits: 2,
                })}
              </Value>
              ...
              <Value secondary="true">
                {formatUSD(editCoveredValue.detectedBalances.usd.toString(), {
                  compact: true,
                  digits: 1,
                })}
              </Value>
            ...
            <MainInfo>
              Current Cover:{' '}
              <Value>
                {formatETH(editCoveredValue.balance.eth.toString(), {
                  compact: true,
                  digits: 2,
                })}
              </Value>
              /
              <Value secondary="true">
                {formatUSD(editCoveredValue.balance.usd.toString(), {
                  compact: true,
                  digits: 1,
                })}
              </Value>
            ...
            <MainInfo>
              Capacity:{' '}
              <Value>
                {formatETH(getCoverAvailable(editCoveredValue.address), {
                  compact: true,
                  digits: 2,
                })}
              </Value>
...
export default withTranslation()(withRouter(withTheme(EditCoveredModal)))
