...
class Arnxm_ARMOR_Uni_TokenContract {
  constructor(service) {
    this.service = service
  }

  shouldDispatch(payload) {
    return this.service.shouldDispatch(Arnxm_ARMOR_Uni_TokenEvents, payload)
  }

  async dispatch(payload) {
    switch (payload.type) {
      case Arnxm_ARMOR_Uni_TokenEvents.GetBalance:
        await this.getBalance(payload)
        break
      case Arnxm_ARMOR_Uni_TokenEvents.GetTotalArmorStaked:
        await this.getTotalArmorStaked(payload)
        break
      case Arnxm_ARMOR_Uni_TokenEvents.Approve:
  ...
  async getBalance(payload) {
    ...
    this.service.setStore({
      Arnxm_ARMOR_Uni_Token_Balance: !response
        ? '0'
        : web3.utils.fromWei(response.toString(), 'ether'),
    })

    this.service.emit(Arnxm_ARMOR_Uni_TokenEvents.BalanceReturned)
    ...
    return new Promise(async (resolve, reject) => {
      try {
        const contract = this.service.makeContract(
          web3,
          this.service.config.arnxm_ARMOR_Uni_Token.abi,
          this.service.config.arnxm_ARMOR_Uni_Token.address
        )
  ...
  async getTotalArmorStaked(payload) {
    ...
    this.service.setStore({
      Arnxm_ARMOR_Uni_Token_TotalArmorStaked: !response
        ? '0'
        : web3.utils.fromWei(response.toString(), 'ether'),
    })
    this.service.emit(Arnxm_ARMOR_Uni_TokenEvents.TotalArmorStakedReturned)
  ...
  async _handleGetTotalArmorStaked(account) {
    ...
    return new Promise(async (resolve, reject) => {
      try {
        ...
        let balance = await contract.methods
          .balanceOf(this.service.config.arnxm_ARMOR_Uni_Farm.address)
          .call({ from: account.address })
  ...
  async approve(payload) {
    ...
    this.service.emit(Arnxm_ARMOR_Uni_TokenEvents.ApproveCompleted, response)
    return response
  }
...
export default Arnxm_ARMOR_Uni_TokenContract
