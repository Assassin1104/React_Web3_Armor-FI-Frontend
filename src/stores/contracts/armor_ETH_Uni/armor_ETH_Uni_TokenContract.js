...
class Armor_ETH_Uni_TokenContract {
  constructor(service) {
    this.service = service
  }

  shouldDispatch(payload) {
    return this.service.shouldDispatch(Armor_ETH_Uni_TokenEvents, payload)
  }

  async dispatch(payload) {
    switch (payload.type) {
      case Armor_ETH_Uni_TokenEvents.GetBalance:
        await this.getBalance(payload)
        break
      case Armor_ETH_Uni_TokenEvents.GetTotalArmorStaked:
        await this.getTotalArmorStaked(payload)
        break
      case Armor_ETH_Uni_TokenEvents.Approve:
  ...
  async getBalance(payload) {
    ...
    this.service.setStore({
      Armor_ETH_Uni_Token_Balance: !response
        ? '0'
        : web3.utils.fromWei(response.toString(), 'ether'),
    })

    this.service.emit(Armor_ETH_Uni_TokenEvents.BalanceReturned)
    ...
    return new Promise(async (resolve, reject) => {
      try {
        const contract = this.service.makeContract(
          web3,
          this.service.config.armor_ETH_Uni_Token.abi,
          this.service.config.armor_ETH_Uni_Token.address
        )
  ...
  async getTotalArmorStaked(payload) {
    ...
    this.service.setStore({
      Armor_ETH_Uni_Token_TotalArmorStaked: !response
        ? '0'
        : web3.utils.fromWei(response.toString(), 'ether'),
    })
    this.service.emit(Armor_ETH_Uni_TokenEvents.TotalArmorStakedReturned)
  ...
  async _handleGetTotalArmorStaked(account) {
    ...
    return new Promise(async (resolve, reject) => {
      try {
        ...
        let balance = await contract.methods
          .balanceOf(this.service.config.armor_ETH_Uni_Farm.address)
          .call({ from: account.address })
  ...
  async approve(payload) {
    ...
    this.service.emit(Armor_ETH_Uni_TokenEvents.ApproveCompleted, response)
    return response
  }
...
export default Armor_ETH_Uni_TokenContract
