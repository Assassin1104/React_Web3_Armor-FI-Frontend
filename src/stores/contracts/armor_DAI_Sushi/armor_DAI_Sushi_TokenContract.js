...
class Armor_DAI_Sushi_TokenContract {
  constructor(service) {
    this.service = service
  }

  shouldDispatch(payload) {
    return this.service.shouldDispatch(Armor_DAI_Sushi_TokenEvents, payload)
  }

  async dispatch(payload) {
    switch (payload.type) {
      case Armor_DAI_Sushi_TokenEvents.GetBalance:
        await this.getBalance(payload)
        break
      case Armor_DAI_Sushi_TokenEvents.GetTotalArmorStaked:
        await this.getTotalArmorStaked(payload)
        break
      case Armor_DAI_Sushi_TokenEvents.Approve:
  ...
  async getBalance(payload) {
    ...
    this.service.setStore({
      Armor_DAI_Sushi_Token_Balance: !response
        ? '0'
        : web3.utils.fromWei(response.toString(), 'ether'),
    })
    this.service.emit(Armor_DAI_Sushi_TokenEvents.BalanceReturned)
    ...
    return new Promise(async (resolve, reject) => {
      try {
        const contract = this.service.makeContract(
          web3,
          this.service.config.armor_DAI_Sushi_Token.abi,
          this.service.config.armor_DAI_Sushi_Token.address
        )
  ...
  async getTotalArmorStaked(payload) {
    ...
    this.service.setStore({
      Armor_DAI_Sushi_Token_TotalArmorStaked: !response
        ? '0'
        : web3.utils.fromWei(response.toString(), 'ether'),
    })
    this.service.emit(Armor_DAI_Sushi_TokenEvents.TotalArmorStakedReturned)
  ...
  async _handleGetTotalArmorStaked(account) {
    ...
    return new Promise(async (resolve, reject) => {
      try {
        ...
        let balance = await contract.methods
          .balanceOf(this.service.config.armor_DAI_Sushi_Farm.address)
          .call({ from: account.address })
  ...
  async approve(payload) {
    ...
    this.service.emit(Armor_DAI_Sushi_TokenEvents.ApproveCompleted, response)
    return response
  }
...
export default Armor_DAI_Sushi_TokenContract
