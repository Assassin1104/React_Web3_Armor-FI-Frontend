...
class ArNXM_ETH_Uni_TokenContract {
  constructor(service) {
    this.service = service
  }

  shouldDispatch(payload) {
    return this.service.shouldDispatch(ArNXM_ETH_Uni_TokenEvents, payload)
  }

  async dispatch(payload) {
    switch (payload.type) {
      case ArNXM_ETH_Uni_TokenEvents.GetBalance:
        await this.getBalance(payload)
        break
      case ArNXM_ETH_Uni_TokenEvents.GetTotalArmorStaked:
        await this.getTotalArmorStaked(payload)
        break
      case ArNXM_ETH_Uni_TokenEvents.Approve:
  ...
  async getBalance(payload) {
    ...
    this.service.setStore({
      ArNXM_ETH_Uni_Token_Balance: !response
        ? '0'
        : web3.utils.fromWei(response.toString(), 'ether'),
    })
    this.service.emit(ArNXM_ETH_Uni_TokenEvents.BalanceReturned)
    ...
    return new Promise(async (resolve, reject) => {
      try {
        const contract = this.service.makeContract(
          web3,
          this.service.config.arNXM_ETH_Uni_Token.abi,
          this.service.config.arNXM_ETH_Uni_Token.address
        )
  ...
  async getTotalArmorStaked(payload) {
    ...
    this.service.setStore({
      ArNXM_ETH_Uni_Token_TotalArmorStaked: !response
        ? '0'
        : web3.utils.fromWei(response.toString(), 'ether'),
    })
    this.service.emit(ArNXM_ETH_Uni_TokenEvents.TotalArmorStakedReturned)
  ...
  async approve(payload) {
    ...
    this.service.emit(ArNXM_ETH_Uni_TokenEvents.ApproveCompleted, response)
    return response
  }
...
export default ArNXM_ETH_Uni_TokenContract
